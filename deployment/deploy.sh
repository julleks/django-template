...
[ $CURRENT_MANIFEST ] && aws ecr put-image --repository-name $ECR_REPOSITORY --image-tag previous-release --image-manifest "$CURRENT_MANIFEST"

aws ecr put-image --repository-name $ECR_REPOSITORY --image-tag current-release --image-manifest "$CANDIDATE_MANIFEST"

[ ! $CURRENT_MANIFEST ] && aws ecr batch-delete-image --repository-name $ECR_REPOSITORY --image-ids imageTag=release-candidate
